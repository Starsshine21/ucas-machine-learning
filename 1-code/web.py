...
model.fit(X_train, y_train, epochs=100, batch_size=32, validation_data=(X_valid, y_valid), callbacks=[early_stopping])
# model = Sequential()
# model.add(Dense(64, input_dim=num_features, activation='relu'))
# model.add(Dropout(0.5))  # 添加Dropout层以减少过拟合
...
# y_valid_pred['pay'] = np.where((tmp['pay'] > 3) & (y_valid_pred['pay'] < 15), 6, y_valid_pred['pay'])
# y_valid_pred['pay'] = np.where((tmp['pay'] > 25) & (y_valid_pred['pay'] < 30), 30, y_valid_pred['pay'])
